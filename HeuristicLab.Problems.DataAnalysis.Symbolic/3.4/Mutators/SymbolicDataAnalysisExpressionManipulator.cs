...
[StorableType("52DC5495-3474-4494-B915-2EF6BD9CA214")]
  public abstract class SymbolicDataAnalysisExpressionManipulator<T> : SymbolicExpressionTreeManipulator, ISymbolicDataAnalysisExpressionManipulator<T> where T : class, IDataAnalysisProblemData {
    ...
    [StorableConstructor]
    protected SymbolicDataAnalysisExpressionManipulator(StorableConstructorFlag _) : base(_) { }
    protected SymbolicDataAnalysisExpressionManipulator(SymbolicDataAnalysisExpressionManipulator<T> original, Cloner cloner)
      : base(original, cloner) {
    }
    public SymbolicDataAnalysisExpressionManipulator()
      : base() {
    ...
    protected static void Swap(CutPoint mutationPoint, ISymbolicExpressionTreeNode selectedBranch) {
      if (mutationPoint.Child != null) {
        // manipulate the tree of parent0 in place
        // replace the branch in tree0 with the selected branch from tree1
        mutationPoint.Parent.RemoveSubtree(mutationPoint.ChildIndex);
        if (selectedBranch != null) {
          mutationPoint.Parent.InsertSubtree(mutationPoint.ChildIndex, selectedBranch);
        }
      } else {
        // child is null (additional child should be added under the parent)
        if (selectedBranch != null) {
          mutationPoint.Parent.AddSubtree(selectedBranch);
        }
      }
    }
  }